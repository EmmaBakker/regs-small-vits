...
class vit_models(nn.Module):
    ...
    with slight modifications
    """
    def __init__(self, img_size=224,  patch_size=16, in_chans=3, num_classes=1000, num_regs=0, embed_dim=768, depth=12,
        ...
        assert num_regs >= 0, 'num_regs should be larger than 0'

        self.num_regs = num_regs
        if num_regs > 0:
            self.reg_tokens = nn.Parameter(torch.zeros(1, self.num_regs, embed_dim))
    ...
    def no_weight_decay(self):
        return {'pos_embed', 'cls_token', 'reg_tokens'}
    ...
    def get_last_selfattention(self, x):
        """
        Computes forward pass but specifically returns raw self-attention of last Transformer block (shape: [B, num_heads, N, N]).
        """
        ...
        # --- IMPORTANT: Insert register tokens if they exist ---
        if self.reg_tokens is not None:
            x = torch.cat(
                (
                    x[:, :1],  # CLS token
                    self.reg_tokens.expand(B, -1, -1),  # register tokens
                    x[:, 1:],  # patch tokens
                ),
                dim=1
            )

        # Go through all blocks except the last one
        ...
        # norm1
        x_norm1 = last_block.norm1(x)
        B_, N_, C_ = x_norm1.shape

        # Project to qkv
        qkv = last_block.attn.qkv(x_norm1) \
            .reshape(B_, N_, 3, last_block.attn.num_heads, C_ // last_block.attn.num_heads)
        ...
        attn = attn.softmax(dim=-1)
        attn_drop = last_block.attn.attn_drop(attn)

        # Continue forward pass for consistency
        x_attn = (attn_drop @ v).transpose(1, 2).reshape(B_, N_, C_)
        x_attn = last_block.attn.proj(x_attn)
        x_attn = last_block.attn.proj_drop(x_attn)

        # Skip-connection
        if hasattr(last_block, "gamma_1"):
            x = x + last_block.drop_path(last_block.gamma_1 * x_attn)
        else:
            x = x + last_block.drop_path(x_attn)

        # norm2 + mlp
        x_norm2 = last_block.norm2(x)
        x_mlp = last_block.mlp(x_norm2)
        if hasattr(last_block, "gamma_2"):
            x = x + last_block.drop_path(last_block.gamma_2 * x_mlp)
        else:
            x = x + last_block.drop_path(x_mlp)

        # Final norm
        x = self.norm(x)

        # attn has shape [B, num_heads, N, N], which LOST expects
        return attn
