...
class vit_models(nn.Module):
    ...
    def get_last_selfattention(self, x):
        ...
        # from the "Attention" submodule:
        B_, N_, C_ = x_norm1.shape
        # project to qkv
        qkv = last_block.attn.qkv(x_norm1).reshape(B_, N_, 3, last_block.attn.num_heads, C_ // last_block.attn.num_heads)
        qkv = qkv.permute(2, 0, 3, 1, 4)  # (3, B, num_heads, N, dim)
        q, k, v = qkv[0], qkv[1], qkv[2]
        
        # raw attention
        attn = (q * last_block.attn.scale) @ k.transpose(-2, -1)  # [B, num_heads, N, N]
        attn = attn.softmax(dim=-1)  # this is the raw self-attention we need

        # after we record attn, we still do the block's forward pass so model is consistent
        ...
        # shape of attn is [B, num_heads, N, N], exactly what LOST wants
        return attn
