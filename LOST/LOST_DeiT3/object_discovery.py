...
import numpy as np
from custom_datasets import bbox_iou
...
def lost(feats, dims, scales, init_image_size, k_patches=100):
    ...
    # Box extraction
    pred, _ = detect_box(
        M, seed, dims, scales=scales, initial_im_size=init_image_size
    )
...
def detect_box(A, seed, dims, initial_im_size=None, scales=None):
    ...
    labeled_array, num_features = scipy.ndimage.label(correl.detach().cpu().numpy() > 0.0)
...
def dino_seg(attn, dims, patch_size, head=0):
    ...
    labeled_array, num_features = scipy.ndimage.label(th_attn[head].detach().cpu().numpy())
