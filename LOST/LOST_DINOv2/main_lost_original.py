...
from networks import get_model  # from the file above
from custom_datasets import Dataset, bbox_iou
...
def main():
    ...
    for im_id, sample in enumerate(pbar):
        ...
        qkv = feat_out["qkv"]
        nb_im = qkv.shape[0]
        nb_tokens = qkv.shape[1]
        n_heads = model.blocks[-1].attn.num_heads

        # Debugging: Log qkv dimensions
        print(f"QKV shape: {qkv.shape}")

        # If qkv is [B, N, 3*dim], reshape to [B, N, 3, nHeads, dim_per_head]
        if qkv.ndim == 3:
            dim_total = qkv.shape[-1]
            embed_dim = dim_total // 3
            dim_per_head = embed_dim // n_heads
            qkv = qkv.reshape(nb_im, nb_tokens, 3, n_heads, dim_per_head)

        # Debugging: Log reshaped qkv dimensions
        print(f"Reshaped QKV shape: {qkv.shape}")

        # permute => (3, B, nHeads, N, dim_per_head)
        ...
        if not args.no_evaluation:
            ...
            if torch.any(ious >= 0.5):
                corloc[im_id] = 1
            cnt += 1
            pbar.set_description(f"Found {int(corloc.sum())}/{cnt}")
